...
class GraphicalLasso(BaseGraphicalLasso):
    ...
    def fit(self, X, aweights=None, y=None):
        ...
        emp_cov = empirical_covariance(X, aweights=self.aweights, assume_centered=self.assume_centered)
...
def graphical_lasso_path(
    ...
    emp_cov = empirical_covariance(X, aweights = train_weights)
    if cov_init is None:
        covariance_ = emp_cov.copy()
    else:
    ...
    if X_test is not None:
        test_emp_cov = empirical_covariance(X_test, aweights = test_weights)
...
class GraphicalLassoCV(BaseGraphicalLasso):
    ...
    aweights: Weights to calculate the empirical covariance matrix
    ...
    def fit(self, X, aweights=None, y=None):
        ...
        aweights: Weights for the empirical covariance matrix
        ...
        #set equal weights if None
        if aweights is None:
            aweights = np.ones(np.shape(X)[0])
        if self.assume_centered:
            self.location_ = np.zeros(X.shape[1])
        else:
            self.location_ = X.mean(0)
            ...
            with warnings.catch_warnings():
                ...
                this_path = Parallel(n_jobs=self.n_jobs, verbose=self.verbose)(
                    delayed(graphical_lasso_path)(
                        X[train],
                        alphas=alphas,
                        train_weights = self.aweights[train],
                        X_test=X[test],
                        test_weights = self.aweights[test],
                        mode=self.mode,
                        tol=self.tol,
                        enet_tol=self.enet_tol,
                    ...
                    for train, test in cv.split(X, y) # instead of y use aweights to split
                )
        ...
        alphas.append(0)
        #no need to add the asweights here. doesnt matter for final estimation i guess!
        grid_scores.append(
            cross_val_score(
                EmpiricalCovariance(),
                X,
